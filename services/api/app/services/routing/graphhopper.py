...
class GraphHopperRoutingAdapter:
    ...
    async def plan_routes(
        ...
        for mode in payload.allowed_modes:
            ...
            if cached_payload:
                available.extend(self._deserialize_candidates(mode=mode, payload=cached_payload))
                continue

            profile, title, comfort_score = profile_metadata
            routes = await self._fetch_routes(profile=profile, payload=payload)
            candidates = [
                RawRouteCandidate(
                    mode=mode,
                    title=title if index == 0 else f"{title} Alt {index}",
                    provider="graphhopper",
                    route_variant="primary" if index == 0 else f"alternative_{index}",
                    path_index=index,
                    distance_meters=float(route["distance"]),
                    base_duration_seconds=float(route["time"]) / 1000,
                    geometry=route["points"],
                    raw_response=route,
                    comfort_score=max(0.55, round(comfort_score - (index * 0.03), 3)),
                )
                for index, route in enumerate(routes[: settings.graphhopper_alternative_paths])
            ]
        ...
        return available[: payload.route_limit], unavailable

    async def _fetch_routes(self, profile: str, payload: MobilityPlanRequest) -> list[dict[str, object]]:
        request_body = {
            "profile": profile,
            "points": [
            ...
            "alternative_route.max_paths": settings.graphhopper_alternative_paths,
    ...
    async def _read_cache(self, redis_client: Redis | None, cache_key: str) -> list[dict[str, object]] | None:
    ...
    async def _write_cache(
        self, redis_client: Redis | None, cache_key: str, candidates: list[RawRouteCandidate]
    ) -> None:
        if redis_client is None:
            return
        try:
                    ...
                    [
                        {
                            ...
                            "route_variant": candidate.route_variant,
    ...
    def _deserialize_candidates(
        self, mode: str, payload: list[dict[str, object]]
        ...
        return [
            RawRouteCandidate(
                ...
                route_variant=str(candidate["route_variant"]),
                path_index=int(candidate["path_index"]),
                distance_meters=float(candidate["distance_meters"]),
                base_duration_seconds=float(candidate["base_duration_seconds"]),
                geometry=dict(candidate["geometry"]),
                raw_response=dict(candidate["raw_response"]),
                comfort_score=float(candidate["comfort_score"]),
            )
